...
$search = isset($_REQUEST['search']) ? $_REQUEST['search'] : 'random';
$pages = 10 + $i;
$urls = 'https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=928c834ae2e13e2e7006bd4f90e7bad3&text=' . $search . '&page=' . $halaman . '&per_page=20&format=rest';


$url = file_get_contents($urls);
$result = simplexml_load_string($url);
$resultphoto = $result->photos->photo;
...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top" style="height: 60px;">
            <div class="container-fluid container">
                ...
                <div class="collapse navbar-collapse p-2 bg-white" id="navbarExample01" style="font-size: large;">
                    ...
                    <!-- Search form -->
                    <form method="GET" action="search.php" class="form-inline d-flex justify-content-center md-form form-sm mt-0 border-bottom" style="height: 30px;">
    ...
    <div class="container">
        <h2 class="px-3 text-dark" style="padding-top: 80px;">Result for <?= $search ?></h2>
        <div class="row row-cols-2 row-cols-lg-3 row-cols-xl-5 g-2 g-lg-4">
            <?php foreach ($resultphoto as $photo) : ?>
                ...
                <ul class="pagination">
                    ...
                    <?php for ($i = $i - 1; $i < $pages; $i++) {
                        if ($i == 0) {
                            ...
                            <li class="page-item"><a class="page-link" style="font-size: large;" href="?search=<?= $search ?>&halaman=<?= $i ?>"><?= $i; ?></a></li>
                    <?php }
                    } ?>
                    <li class="page-item disabled">
